...
drop trigger if exists trig_set_updated_at on public.issues;
...
-- Drop existing policies first (safe to fail)
...
drop policy if exists "issues: read all" on public.issues;
drop policy if exists "issues: insert own" on public.issues;
drop policy if exists "issues: update by officials" on public.issues;
drop policy if exists "issue_images: read all" on public.issue_images;
drop policy if exists "issue_images: insert by issue owner" on public.issue_images;
drop policy if exists "votes: read all" on public.votes;
drop policy if exists "votes: insert auth" on public.votes;
...
create policy "departments: read all" on public.departments for select using (true);
...
-- Anonymous users can also insert issues (for non-authenticated reporting)
...
create policy "issue_images: read all" on public.issue_images for select using (true);
create policy "issue_images: insert by issue owner" on public.issue_images
for insert with check (
...
-- Anonymous image uploads allowed
...
create policy "votes: insert auth" on public.votes
for insert with check (
  auth.role() = 'authenticated' and voter_id = auth.uid()
);

-- Anonymous voting allowed (IP-based dedup)
create policy "votes: insert anonymous" on public.votes
for insert with check (voter_id is null);
...
create policy "status_history: read all" on public.status_history for select using (true);
